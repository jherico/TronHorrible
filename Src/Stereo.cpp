...
StereoConfig::StereoConfig(StereoMode mode, const Viewport& vp) :
		Mode(mode), InterpupillaryDistance(0.064f), AspectMultiplier(1.0f), FullView(
				vp), DirtyFlag(true), IPDOverride(false), YFov(0), Aspect(
				vp.w / float(vp.h)), ProjectionCenterOffset(0), OrthoPixelOffset(
				0) {
...
void StereoConfig::SetHMDInfo(const HMDInfo& hmd) {
	...
	Distortion.SetChromaticAberration(hmd.ChromaAbCorrection[0],
			hmd.ChromaAbCorrection[1], hmd.ChromaAbCorrection[2],
...
const StereoEyeParams& StereoConfig::GetEyeRenderParams(StereoEye eye) {
	static const unsigned char eyeParamIndices[3] = { 0, 0, 1 };
...
void StereoConfig::update2D() {
	...
	float lensDistanceScreenPixels = metersToPixels
			* HMD.LensSeparationDistance;
	float eyeDistanceScreenPixels = metersToPixels * InterpupillaryDistance;
	float offCenterShiftPixels = (HMD.EyeToScreenDistance / 0.8f)
			* eyeDistanceScreenPixels;
	float leftPixelCenter = (HMD.HResolution / 2)
			- lensDistanceScreenPixels * 0.5f;
	...
	float percievedHalfScreenDistance = tan(Area2DFov * 0.5f)
...
void StereoConfig::updateEyeParams() {
	...
	switch (Mode) {
	case Stereo_None: {
		EyeRenderParams[0].Init(StereoEye_Center, FullView, 0, projCenter,
				OrthoCenter);
	}
		break;
